class SceneMain extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("sprBoss", "content/sprBoss.png", {
      frameWidth: 200,
      frameHeight: 154
    });
    ...
    this.load.audio("sndHit", "content/hit.ogg");
  }
  ...
  create() {
    ...
    this.time.addEvent({
      ...
      callback: function() {
        ...
        if (score > 200 && boss == null && oneBoss == false) {
          boss = new BossShip(
            this,
            this.game.config.width * 0.5,
            45
    ...
    this.physics.add.overlap(this.playerLasers, this.bosses, function(playerLaser, boss) {
      playerLaser.destroy();
      if (boss) {
        console.log("x" + boss.body.x);
        console.log("y" + boss.body.y);
        if (boss.onDestroy !== undefined) {
          var tmp = boss.getData("blood");
          console.log(tmp);
          boss.setData("blood", tmp-1);
        }
        if(boss.getData("blood")<=0) {
          boss.explode(true);
          score += 40;
          scoreText.setText('Score: ' + score);
        }
    ...
    this.physics.add.overlap(this.player, this.bosses, function(player, boss) {
  ...
  */
  ...
  update() {
    ...
    for (var i = 0; i < this.bosses.getChildren().length; i++) {
      var boss = this.bosses.getChildren()[i];
      boss.update();
    }
